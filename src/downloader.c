...
/* Replace character in a string with the specified character:
*
*/
...
size_t file_size(int file_desc)
{
    struct stat sb;
...
{
    // For any error we will simply leave the files as is.
    char file_dir[256];
    char *mm_dest;
    char *mm_src;
    size_t d_fs, s_fs;
    ...
    // replace the slash character for the file_name
    remove_char(dest, "/", "_");
    sprintf(file_dir, "%s/%s", src, dest);
    dfile = open(file_dir, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (dfile == -1)
        return;
    d_fs = tasks * bytes;
    if (ftruncate(dfile, d_fs))
        return;

    mm_dest = mmap(NULL, d_fs, PROT_READ | PROT_WRITE, MAP_SHARED, dfile, 0);
    if (mm_dest == MAP_FAILED)
        return;

    printf("* Copy chunks to file at %s\n", file_dir);

    for (int i = 0; i < tasks; i++)
    {
        sprintf(file_dir, "%s/%d", src, i * bytes);
        sfile = open(file_dir, O_RDONLY);
        s_fs = file_size(sfile);
        mm_src = mmap(NULL, s_fs, PROT_READ, MAP_PRIVATE, sfile, 0);
        if (mm_src == MAP_FAILED)
            return;
        memcpy(mm_dest + i * bytes, mm_src, s_fs);
        munmap(mm_src, s_fs);
        close(sfile);
        remove(file_dir);
    }
...
void remove_chunk_files(char *dir, int bytes, int files)
{
    ...
    char rem_dir[20];
    for (int file_i = 0; file_i < files; file_i++)
    {
        sprintf(rem_dir, "%s/%d", dir, file_i * bytes);
        remove(rem_dir);
    }
}
...
int main(int argc, char **argv)
{
    ...
    while ((len = getline(&line, &len, fp)) != -1)
    {
         ...
         */
        m_map_merge_files(download_dir, line, bytes, num_tasks);
    }
